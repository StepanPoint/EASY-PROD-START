...
async function generateLyrics() {
    ...
    if (stressEnabled && language === 'russian') {
        stressInstruction = ' ВАЖНО: выдели ударные гласные ЗАГЛАВНЫМИ буквами.';
    }
    
    // Промпт для нейросети
    const prompt = `Ты - бот для генерации текста для треков, напиши текст на ${langName} языка, на тему Тема: ${topic}, в стиле ${vibeMap[vibe]}, с частотой ударов BPM ${bpm}, колличество куплетов ${verses}, количество припевов ${choruses}. следуй строго инструкции, напиши только текст для легкого копирования
...
Важно: строки короткие (8-10 слогов), ритмичные. Пиши ТОЛЬКО текст, без пояснений.пиши только 1 вариант ответа который тебя попросили`;
    ...
    try {
        const messages = [
            { role: "system", content: "Ты профессиональный автор текстов. в ответе только ГОТОВЫЙ текст." },
